...
class User extends CommonObject
{
	...
	public function fetch($id = '', $login = '', $sid = '', $loadpersonalconf = 0, $entity = -1, $email = '')
	{
        ...
        $sql .= " u.google_url as google_url,";
		...
		if ($result) {
			...
			if ($obj) {
				...
				$this->google_url   = $obj->google_url;
	...
	public function hasOwnStockLabels() {
	    $sql = "SELECT stock_correction_labels FROM ".MAIN_DB_PREFIX."user_extrafields";
        ...
        $obj = $this->db->fetch_object($result);
        if (!$obj || !$obj->stock_correction_labels) {
            return false;
        }
        $select = '<select class="button buttongen" name="label" id="label">';
        $options = explode(';',$obj->stock_correction_labels);
        if (count($options)<2) return false;
        foreach ($options as $option) {
            [$value,$display] = explode('#',$option);
    ...
    public function sourceWarehouses() {
	    // warehouse manager can only send products from his own warehouses
