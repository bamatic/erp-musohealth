...
if (empty($reshook))
{
	...
	if ($action == 'add' && $user->rights->stock->creer)
	{
        ...
        $object->fk_departement     = (string) GETPOST("fk_departement",'alpha');
	...
	if ($action == 'update' && $cancel <> $langs->trans("Cancel"))
	{
		if ($object->fetch($id))
		{
            ...
            $object->fk_departement     = GETPOST("fk_departement");
...
// State
    ...
    {
        print '<tr><td>'.$form->editfieldkey('Region-State', 'fk_departement', '', $object, 0).'</td><td colspan="3" class="maxwidthonsmartphone">';
    } else {
        print '<tr><td>'.$form->editfieldkey('State', 'fk_departement', '', $object, 0).'</td><td colspan="3" class="maxwidthonsmartphone">';
    }

    if ($mysoc->country_code) print $formcompany->select_state($object->fk_departement, $mysoc->country_code,'fk_departement');
	...
	if ($id > 0 || $ref)
	{
		...
		if ($action <> 'edit' && $action <> 're-edit')
		{
			...
			if ($user->rights->stock->mouvement->creer && $object->hasTransfertPermission($user)) {
				print_liste_field_titre('');
			}
			if ($user->rights->stock->mouvement->creer && $object->hasCorrectionPermission($user)) {
				print_liste_field_titre('');
			}
			print "</tr>\n";
...
// State
                ...
                if ($object->country_id) print $formcompany->select_state($object->fk_departement, $object->country_code,'fk_departement');
                else print $countrynotdefined;
                print '</td></tr>';
