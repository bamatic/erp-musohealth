...
print '<script type="text/javascript" language="javascript">
		jQuery(document).ready(function() {
			...
			jQuery( "#musosave" ).click(function(event) {
			    event.preventDefault();
			    var label = jQuery("#label").val();
			    if (label == "MODIFIER")
                    alert("la libelle du mouvement est obligaotire ");
			    else 
			        jQuery("#musomouvement").submit();
            });
		});
		</script>';
...
print '<form action="'.$_SERVER["PHP_SELF"].'?id='.$id.'" method="post" id="musomouvement">'."\n";
...
if ($object->element == 'product') {
	...
	if ($user->fk_user ||  $user->id == 6) {
        print '<option value="2" selected>' . $langs->trans("Delete") . '</option>';
    }
	else {
...
print '</tr>';
*/
$labelSelect = $user->hasOwnStockLabels();
...
print '<tr>';
print '<td class="fieldrequired">'.$langs->trans("MovementLabel").'</td>';
print '<td>';
if ($labelSelect === false) {
...
print '<input type="submit" class="button button-save" name="save" id="musosave" value="'.dol_escape_htmltag($langs->trans("Save")).'">';
print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
