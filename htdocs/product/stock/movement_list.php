...
if ($resql)
{
	...
	{
	    if ($object->hasCorrectionPermission($user))
	...
	if ($action == "transfert")
	{
        if ($object->hasTransfertPermission($user))
	...
	while ($i < min($num, $limit))
	{
		...
		if (!empty($arrayfields['m.type_mouvement']['checked']))
		{
			...
			switch ($objp->type_mouvement) {
				case "0":
					print '<td class="center">'.$langs->trans('StockIncreaseAfterTransfer').'</td>';
					break;
				case "1":
					print '<td class="center">'.$langs->trans('StockDecreaseAfterTransfer').'</td>';
					break;
				case "2":
