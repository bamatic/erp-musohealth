...
class Entrepot extends CommonObject
{
    ...
    */
	...
	public $fields = array(
		...
		'town' =>array('type'=>'varchar(50)', 'label'=>'Town', 'enabled'=>1, 'visible'=>1, 'position'=>55, 'searchall'=>1),
		'fk_departement' =>array('type'=>'sellist:c_departements:nom:rowid::active=1 AND fk_country=147', 'label'=>'Site', 'enabled'=>0, 'visible'=>0, 'position'=>60),
	...
	public function create($user)
	{
		...
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."entrepot (ref, entity, datec, fk_user_author, fk_parent, fk_departement)";
		$sql .= " VALUES ('".$this->db->escape($this->label)."', ".$conf->entity.", '".$this->db->idate($now)."', ".$user->id.", ".($this->fk_parent > 0 ? $this->fk_parent : "NULL").", ".(intval($this->fk_departement) > 0 ? $this->fk_departement : "NULL").")";
	...
	public function update($id, $user)
	{
        ...
        $sql .= ", fk_departement = ".((intval($this->fk_departement) > 0) ? $this->fk_departement : "NULL");
	...
	public function fetch($id, $ref = '')
	{
		...
		$sql  = "SELECT rowid, entity, fk_parent, ref as label, description, statut, lieu, address, zip, town, fk_pays as country_id, fk_departement, phone, fax,";
	...
	public function getSite() {
	    if (!$this->fk_departement) return null;
        $sql  = "SELECT nom FROM llx_c_departements";
        $sql .= " WHERE rowid = '".$this->fk_departement."'";
    ...
    public function hasTransfertPermission($user) {
	    if ($user->admin) return true;
	    $sql = "SELECT stock_user from llx_entrepot_extrafields";
        ...
        if ($result)
        {
            if ($this->db->num_rows($result) == 1)
            {
               ...
               return ( $obj->stock_user == $user->id  && !$user->fk_user);
    ...
    public function hasCorrectionPermission($user) {
        if ($user->admin) return true;
        $sql = "SELECT stock_user from llx_entrepot_extrafields";
        $sql .= " WHERE fk_object =" . $this->id;
        $result = $this->db->query($sql);
