...
class FormProduct
{
     ...
     * @param   int         $transfer           0=correction, 1 source, 2 destionation
	 * @return  int                             Nb of loaded lines, 0 if already loaded, <0 if KO
	 * @throws  Exception
	 */
	public function loadWarehouses($fk_product = 0, $batch = '', $status = '', $sumStock = true, $exclude = '', $stockMin = false, $orderBy = 'e.ref',$transfer = 0)
	{
		...
		$sql = "SELECT e.rowid, e.ref as label, e.description, e.fk_parent, eef.stock_user";
		...
		$sql .= " INNER JOIN ".MAIN_DB_PREFIX."entrepot_extrafields eef on e.rowid = eef.fk_object";
		...
		if ($transfer == 0) { //correction
            if ($user->fk_user) // has parents so he sees only his own warehouses
                $sql .= " AND eef.stock_user=" . $user->id;
            elseif ($user->state_id)  //hs no parent user he is the chef of his area/state show all warehouses into his state/area
                $sql .= " AND e.fk_departement=" . $user->state_id;
        }
		elseif ($transfer == 1) { //source
            $sql .= " AND eef.stock_user=" . $user->id;
        }
		elseif ($transfer == 2) { // destination
            if ($user->state_id)  // all wwarehouses into his state/area
                $sql .= " AND e.fk_departement=" . $user->state_id;
        }
		...
		if ($sumStock && empty($fk_product)) {
			$sql .= " GROUP BY e.rowid, e.ref, e.description, e.fk_parent, eef.stock_user";
	 ...
	 *  @throws Exception
	 */
	public function selectWarehouses($selected = '', $htmlname = 'idwarehouse', $filterstatus = '', $empty = 0, $disabled = 0, $fk_product = 0, $empty_label = '', $showstock = 0, $forcecombo = 0, $events = array(), $morecss = 'minwidth200', $exclude = '', $showfullpath = 1, $stockMin = false, $orderBy = 'e.ref', $transfer = 0)
	{
		...
		$this->loadWarehouses($fk_product, '', $filterstatus, true, $exclude, $stockMin, $orderBy, $transfer);
