...
if (!defined('NOLOGIN'))
{
	...
	if (!isset($_SESSION["dol_login"]))
	{
        ...
        if (isset($_GET["muso"]) && $_GET['muso'] == 'google' && in_array('muso', $authmode)) {
            $goontestloop = true;
            $usertotest = $_GET['code'];
            $passwordtotest = 'muso';
            $entitytotest = 1;
        }
	...
	if (empty($disablehead))
	{
        ...
        if (strpos($_SERVER['PHP_SELF'],'/ticket/card.php') !== false) {
            ...
            print '
                    ...
                    <link href="/public/ticket/unitegallery/themes/default/ug-theme-default.css" rel="stylesheet">
                    <script src="https://cdn.rawgit.com/mattdiamond/Recorderjs/08e7abd9/dist/recorder.js"></script>
  	                <script src="/public/ticket/recorder/recorder-widget.js"></script>
            ';
        }
...
function top_menu_user($hideloginname = 0, $urllogout = '')
{
	...
	if (!empty($user->photo))
	{
		...
		$userDropDownImage = '<img class="photo dropdown-user-image" alt="No photo" src="'.DOL_URL_ROOT.$nophoto.'">';
         ...
         if ( $user->google_url) {
                $dom = new DOMDocument();
                $dom->loadHTML($userImage);
                $images = $dom->getElementsByTagName('img');
                if (count($images)) {
                    $img = $images[0];
                    $src = $img->getAttribute('src');
                    if ($src) {
                        $userImage = str_replace($src,$user->google_url,$userImage);
                    }
                }
                $dom->loadHTML($userDropDownImage);
                $images = $dom->getElementsByTagName('img');
                if (count($images)) {
                    $img = $images[0];
                    $src = $img->getAttribute('src');
                    if ($src) {
                        $userDropDownImage = str_replace($src,$user->google_url,$userDropDownImage);
                    }
                }
            }
	$dropdownBody = '';
