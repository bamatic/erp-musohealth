	...
	|| $action == 'editsubject' || $action == 'edit_extras' || $action == 'update_extras' || $action == 'edit_extrafields' || $action == 'set_extrafields' || $action == 'classify' || $action == 'sel_contract' || $action == 'edit_message_init' || $action == 'set_status' || $action == 'dellink')
{
	if ($res > 0)
	{
		...
		$medias = $object->mediaLinks();

		print $object->videoGallery($medias);
		print '
		    <script type="text/javascript">

                jQuery(document).ready(function(){
        
                    jQuery("#ticket-gallery").unitegallery();
        ...
        print '<table class="border tableforfield centpercent">';
        print '<tr>';
        print '<td id="received-audios">';
        print $object->audios($medias);
        print  '</td>';
        print '<td id="sent-audios">';

        print  '</td>';
        print '</tr>';
        //a row for record whatsapp msg
        print '<tr>';
        print '<td id="audio-widget" colspan="2">';
        print '<div id="controls">
  	            <button id="recordButton" class="button">Record <i class="fa fa-microphone" style="color:red"></i></button>
  	            <button id="pauseButton" class="button" disabled>Pause <i class="fa fa-pause"></i></button>
  	            <button id="stopButton"  class="button" disabled>Stop <i class="fa fa-stop"></i></button>
            </div>
            <div id="animation" >
                <img id="gif-recording" src="/public/ticket/recorder/img/recording.gif" style="max-height: 35px; width: 80%;display: none;" alt="recording">
                <img id="gif-pause" src="/public/ticket/recorder/img/recording-pause.gif" style="max-height: 35px; width: 80%; display: none;" alt="recording">
            </div>';
        print  '</td>';
        print '</tr>';
        print '</table>';
		    ...
		    <script type="text/javascript">
                ...
                jQuery(document).ready(function(){
                   ...
                   URL = window.URL || window.webkitURL;
                    ...
                    var input; 							//MediaStreamAudioSourceNode we will be recording
                    
                    // shim for AudioContext when it is not avb. 
