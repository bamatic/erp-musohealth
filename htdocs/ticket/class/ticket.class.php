...
class Ticket extends CommonObject
{
    ...
    private function extension($url) {
	    if ( ($pos = strrpos($url,'.') ) === false) return '';
	    if ($pos == strlen($url) - 1) return '';
	    return substr($url,$pos+1);
    }
    public function mediaLinks()
    {
        $sql = 'SELECT url,datea FROM ' . MAIN_DB_PREFIX . 'links';
        $sql .= ' WHERE objecttype="ticket" AND objectid=' . $this->id;
        ...
        for ($i=0; $i < $num; $i++) {
            ...
            $extension = $this->extension($obj->url);

            if (in_array($extension,array_keys($media))) {
                $media[$extension][] = ["date"=>$obj->datea,"url"=>$obj->url];
            }
        }
        return $media;
    ...
    public function printMedia($media)
    {
        ...
        foreach ($media['ogg'] as $audio) {
            $html .= '
                <div>
                    <div style="display: inline-block; text-align: left"><small>'.  date_format(new DateTime($audio["date"]),"d/m/Y H:i:s") . '</small></div>
    ...
    public function audios($media) {
        ...
        foreach ($media['ogg'] as $audio) {
            $html .= '
                <div>
                    <div style="display: inline-block; text-align: left"><small>'.  date_format(new DateTime($audio["date"]),"d/m/Y H:i:s") . '</small></div>
                    <div style="display: inline-block; text-align: right; max-height: 15px">
                        <audio controls>
                            <source src=" ' . $audio["url"] . '">
                        </audio><br>
                    </div>
    ...
    public function videoGallery($media)
    {
        if (!$media) return '<div id="ticket-gallery" style="display:none;"></div>';
        $div = '<div id="ticket-gallery" style="display:none;">';
        foreach ($media["jpeg"] as $i=>$jpeg) {
            $div .= '<img alt="Ticket Image ' . $i . '"';
            $div .= 'src="' . $jpeg["url"] . '"';
            $div .= 'data-image="' . $jpeg["url"] . '">';
        }
        foreach ($media["mp4"] as $i=>$mp4) {
            $div .= '<img alt="Ticket video ' . $i . '"';
            $div .= 'src="' . $mp4["url"] . '"';
            $div .= 'data-type="html5video"';
            $div .= 'data-videomp4="' . $mp4["url"] . '">';
        }
        return $div . '</div>';
    }
