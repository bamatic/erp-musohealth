...
class Tickets extends DolibarrApi
{
    ...
    {
        $sql = "SELECT l.rowid, l.url, l.datea, l.objectid, l.label";
        $sql .= " FROM llx_links l";
        $sql .= " WHERE l.objecttype='ticket'";
        ...
        if ($result) {
            ...
            while ($i < $num) {
                ...
                $obj_ret[] = [
                    ...
                    "label"=>$obj->label,
                    "datea"=>$obj->datea,
                    "ticket_id"=>$obj->objectid
                ];
                $i++;
            }
        } else {
            throw new RestException(503, 'Error when retrieve ticket link list');
        }
        if (!count($obj_ret)) {
            throw new RestException(404, 'No ticket found');
        }
    ...
    /**
     * add a link to a given ticket
     *
     * @param   int     $id   Ticket ID
     * @return  array
     * @url POST /links
     */
    public function addLink($id,$url,$label)
    {
        $sql = "INSERT INTO llx_links ( url, datea, objectid, label, objecttype, entity) VALUES";
        $sql .= " ('" . $url . "','" . date('Y-m-d h:i:s') . "'," . $id . ",'" . $label . "', 'ticket', 1" .")";
        ...
        if ($result) {
            ...
            die($sql);
            throw new RestException(500, 'Unable to create ticket');
        }
